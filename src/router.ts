...
export async function routeInbound(event: InboundEvent): Promise<void> {
  ...
  if (!found) {
    ...
    mg = {
      ...
      // Inherited channels copy policy from the template; everything else
      // gets the cautious default (forces channel-request approval flow).
      unknown_sender_policy: template?.mg.unknown_sender_policy ?? 'request_approval',
      denied_at: null,
